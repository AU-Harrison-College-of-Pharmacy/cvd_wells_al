...
tab_footnote(
 footnote = "Proportion that posterior samples in which all interaction parameter estimates are within the ROPE interval [-0.1, 0.1] on the log scale",
...
read_rds("/Volumes/Projects/usgs_cvd_wells_al/output/09_p_dir_rope_cbg_pct_aa_only_inla.rds") %>%
  rename(p_dir_pct_wells_cbg = p_dir,
         ROPE_pct_wells_cbg = ROPE) %>%
    pivot_longer(
        cols = c(p_dir_pct_wells_cbg, ROPE_pct_wells_cbg),
        names_to = "term",
        values_to = "value"
    ) %>% pivot_wider(names_from = Condition) %>%
  rename(`Stroke/cerebrovascular` = Stroke_cerebrovascular) %>%
