...
write_rds(f_hypertensive_ixn_age_group, "/Volumes/Projects/usgs_cvd_wells_al/output/04_hypertension_deaths_poisson_model_ixn_age_group.rds")

# Ischemic

f_ischemic_ixn_age_group <-  lapply(1:length(df_ischemic$imputations), function(i){
  f <- glmer(n_ischemic_deaths ~ (1 | id_census_block_group) +  amt_centered_scaled_mean_pct_wells_cbg * cat_age_group + cat_rural + offset(log(n_population_times_4)),
...
write_rds(f_ischemic_ixn_age_group, "/Volumes/Projects/usgs_cvd_wells_al/output/04_ischemic_deaths_poisson_model_ixn_age_group.rds")

# Stroke/cerebrovascular

f_stroke_cerebrovascular_ixn_age_group <- lapply(1:length(df_stroke_cerebrovascular$imputations), function(i){
  f <- glmer(n_stroke_cerebrovascular_deaths ~  (1 | id_census_block_group) + amt_centered_scaled_mean_pct_wells_cbg * cat_age_group + cat_rural + offset(log(n_population_times_4)),
...
write_rds(f_stroke_cerebrovascular_ixn_age_group, "/Volumes/Projects/usgs_cvd_wells_al/output/04_stroke_cerebrovascular_deaths_poisson_model_ixn_age_group.rds")

# Diabetes

f_diabetes_ixn_age_group <- lapply(1:length(df_diabetes$imputations), function(i){
  f <- glmer(n_diabetes_deaths ~  (1 | id_census_block_group) +  amt_centered_scaled_mean_pct_wells_cbg * cat_age_group + cat_rural + offset(log(n_population_times_4)),
...
write_rds(f_diabetes_ixn_age_group, "/Volumes/Projects/usgs_cvd_wells_al/output/04_diabetes_deaths_poisson_model_ixn_age_group.rds")
