...
f_hypertensive_ixn_age_group <- lapply(1:length(df_hypertensive$imputations), function(i){
  f <- inla(
    n_hypertensive_deaths ~ f(id_census_block_group, model = "iid") + amt_centered_scaled_mean_pct_wells_cbg * cat_age_group + cat_rural + offset(log(n_population_times_4)),
...
pool_inla(f_hypertensive_ixn_age_group) %>%
  write_rds("/Volumes/Projects/usgs_cvd_wells_al/output/04_03_pool_hypertension_deaths_poisson_model_ixn_age_group_inla.rds")
...
write_rds(preds_hypertensive, "/Volumes/Projects/usgs_cvd_wells_al/output/04_03_preds_hypertension_deaths_poisson_model_ixn_age_group_inla.rds")

# Ischemic

f_ischemic_ixn_age_group <-  lapply(1:length(df_ischemic$imputations), function(i){
  f <- inla(n_ischemic_deaths ~ f(id_census_block_group, model = "iid") +  amt_centered_scaled_mean_pct_wells_cbg * cat_age_group + cat_rural + offset(log(n_population_times_4)),
...
pool_inla(f_ischemic_ixn_age_group) %>%
  write_rds("/Volumes/Projects/usgs_cvd_wells_al/output/04_03_pool_ischemic_deaths_poisson_model_ixn_age_group_inla.rds")

preds_ischemic <- pool_predictions_inla(f_ischemic_ixn_age_group, 
                                        terms = c("amt_centered_scaled_mean_pct_wells_cbg [-0.78:2.9]", "cat_age_group"),
                                        condition = c(n_population_times_4 = 100000))

write_rds(preds_ischemic, "/Volumes/Projects/usgs_cvd_wells_al/output/04_03_preds_ischemic_deaths_poisson_model_ixn_age_group_inla.rds")

# Stroke/cerebrovascular

f_stroke_cerebrovascular_ixn_age_group <- lapply(1:length(df_stroke_cerebrovascular$imputations), function(i){
  f <- inla(n_stroke_cerebrovascular_deaths ~  f(id_census_block_group, model = "iid") + amt_centered_scaled_mean_pct_wells_cbg * cat_age_group + cat_rural + offset(log(n_population_times_4)),
...
pool_inla(f_stroke_cerebrovascular_ixn_age_group) %>%
  write_rds("/Volumes/Projects/usgs_cvd_wells_al/output/04_03_pool_stroke_cerebrovascular_deaths_poisson_model_ixn_age_group_inla.rds")

preds_stroke_cerebrovascular <- pool_predictions_inla(f_stroke_cerebrovascular_ixn_age_group, 
                                                      terms = c("amt_centered_scaled_mean_pct_wells_cbg [-0.78:2.9]", "cat_age_group"),
                                                      condition = c(n_population_times_4 = 100000))

write_rds(preds_stroke_cerebrovascular, "/Volumes/Projects/usgs_cvd_wells_al/output/04_03_preds_stroke_cerebrovascular_deaths_poisson_model_ixn_age_group_inla.rds")

# Diabetes

f_diabetes_ixn_age_group <- lapply(1:length(df_diabetes$imputations), function(i){
  f <- inla(n_diabetes_deaths ~  f(id_census_block_group, model = "iid") +  amt_centered_scaled_mean_pct_wells_cbg * cat_age_group + cat_rural + offset(log(n_population_times_4)),
...
pool_inla(f_diabetes_ixn_age_group) %>%
  write_rds("/Volumes/Projects/usgs_cvd_wells_al/output/04_03_pool_diabetes_deaths_poisson_model_ixn_age_group_inla.rds")

preds_diabetes <- pool_predictions_inla(f_diabetes_ixn_age_group, 
                                        terms = c("amt_centered_scaled_mean_pct_wells_cbg [-0.78:2.9]", "cat_age_group"),
                                        condition = c(n_population_times_4 = 100000))

write_rds(preds_diabetes, "/Volumes/Projects/usgs_cvd_wells_al/output/04_03_preds_diabetes_deaths_poisson_model_ixn_age_group_inla.rds")
