...
adph_primary <- read_csv("/Volumes/Projects/usgs_cvd_wells_al/data/raw/2016-2019CensusBlockGroupPrimaryCOD.CSV",
                         col_types = c("c", "c", "i", "i", "i", "i")) %>%
  janitor::clean_names() %>%
  rename() %>%
  mutate(cbg_included_by_adph = 1,
         census_block_group = as.character(census_block_group),
         census_block_group = paste0("0", census_block_group))  # The ADPH data is missing the leading '0' that is usually associated with a state FIPS code ('01' for Alabama)
...
df_adph_included <- filter(df, cbg_included_by_adph == 1) %>%
  as_tibble() %>%
  select(census_block_group, age_group, hypertensive_deaths, ischemic_deaths, stroke_cerebrovascular_deaths, diabetes_deaths, population_estimate, geometry)
