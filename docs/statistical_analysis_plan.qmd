...
The [*overarching objective*]{.underline} of this research project is to determine how much well water use rates in Census block groups (CBGs) may be related to cardiovascular disease (CVD) mortality in those CBGs in Alabama. Our rationale for pursuing this objective is that CVD mortality is a major public health concern in Alabama, and the contribution of private well drinking water contamination to CVD mortality is not well understood. Because CVD mortality is generally more common in rural areas of Alabama, and some of these rural areas have higher rates of private well use, contamination of drinking water among those with private wells may contribute to geographic variation in Alabama’s rates of CVD mortality.
...
-   **Population counts for CBGs in Alabama for 2000, 2010, and 2020** These counts will be obtained from the decennial census using the `tidycensus` package in `R`. The counts come from different variables in the `dhc` summary file, and they are split by age group and sex. We have to combine the appropriate variables in order to arrive at total population estimates for each of the CBGS.

# Key variables

## Outcomes

-   Number of deaths from ischemic heart disease in a given CBG from 2016 - 2019
-   Number of deaths from hypertensive heart disease in a given CBG from 2016 - 2019
-   Number of deaths from cerebrovascular diseases/stroke in a given CBG from 2016 - 2019
-   Number of deaths from diabetes in a given CBG from 2016 - 2019

## Exposures

- Percent of population served by domestic wells in a given CBG in 2020
    - One challenge with this variable is that 0% is the 1st quartile and the standard deviation is 27% with a mean of 21%. That means that when trying to center and scale the variable, going one standard deviation down from the mean is already an impossible number. So for the negative values of the scaled variable, they will have a hard boundary at ~`-0.778`.
...
-   Predictors of outcome: age group (`45 - 54`, `55 - 64`, `65 - 74`, and `75+`)
...
```{=typst}
$ "Ischemic Heart Disease Deaths"_("ij") ~ "Poisson"(lambda_("ij")) $
$ ln(lambda_("ij")) = beta_0 + "CBG"_i + W_("i")beta_1 + R_i beta_2 + bold(A)_("ij") bold(beta)_3 + ln("population"_("ij") * 4) $
$ "CBG" ~ N(0, sigma_C) $

/ $"CBG"_i$: the $i^("th")$ Census block group 
/ $W_("i")$: the percentage of housing units estimated to be using private wells in the $i^("th")$ Census block group
/ $R_i$: the rurality of the $i^("th")$ Census block group
/ $bold(A)_("ij")$: the age group, treated as a categorical variable
/ $"population"_("ij") * 4$: the residential population of the $i^("th")$ Census block group in the $j^("th")$ age group. This number is multiplied by 4 since the counts for numbers of deaths are over 4 years. We are assuming the population in 2020 is the same as the population in each of the years under study.
```
...
| cbg          | year | age_group | population | cbg_rurality | percentage_well_users | ischemic_deaths |
|-----------|-----------|-----------|-----------|-----------|-----------|-----------|
| 010810407002 | 2019 | 45 - 54   | 600        | 1            | 5                 | 0               |
| 010810407002 | 2019 | 55 - 64   | 1000       | 1            | 5                 | 3               |
| 010810407002 | 2019 | 65 - 74   | 1200       | 1            | 5                 | 10              |
...
a_df <- amelia(as.data.frame(a) %>%
                 ...
                 dplyr::select(-c(geometry, population_estimate, mean_pct_wells_cbg)),
               m = 60,
...
f_imputed <- lapply(1:2, function(i){
  f <- glmer(round(hypertensive_deaths) ~ (1 | census_block_group) + age_group + ns(c) + offset(log(p)),
             family = poisson(link = "log"),
             data = a_df$imputations[[i]] %>% filter(!(census_block_group %in% c("010730027011", "010810406031"))) %>% # These two census block groups have population of 0, which was throwing off the model. So we are excluding them. %>%
               mutate(age_group = as_factor(age_group)),  # These deaths are aggregating over a certain number of years, so we are multiplying the population by the number of years to reflect the real population at risk for that many deaths. This number may be corrected later. It's currently yielding approximately the correct rates.
             glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 500000))
             )
})
...
predictions_ischemic <- lapply(imputed_models, predict_response, terms = c("c_wells [0, 1]"),
                               condition = c(population = 100000, rural = 1, age_group = "65 plus")) %>%
...
```{=typst}
$ "Ischemic Heart Disease Deaths"_("ij") ~ "Poisson"(lambda_("ij")) $
$ ln(lambda_("ij")) = beta_0 + "CBG"_i + W_("i")beta_1 + R_i beta_2 + bold(A)_("ij") bold(beta)_3 + bold("Region")_i bold(beta)_4 + W_("i") "x " bold("Region")_i bold(beta)_5 + ln("population"_("ij") * 4) $
$ "CBG" ~ N(0, sigma_C) $
...
```{=typst}
$ "Ischemic Heart Disease Deaths"_("ij") ~ "Poisson"(lambda_("ij")) $
$ ln(lambda_("ij")) = beta_0 + "CBG"_i + W_("i")beta_1 + R_i beta_2 + bold(A)_("ij") bold(beta)_3 + L_i bold(beta)_4 + W_("i") "x " L_i bold(beta)_5 + ln("population"_("ij") * 4) $
$ "CBG" ~ N(0, sigma_C) $
...
|----------|------|----------------|
| Request ages aggregated across years         |  2024-12    |  Matthew Loop (matthew_loop@auburn.edu)              |
|          |      |                |
|          |      |                |
